-- Ruta Original: ReplicatedStorage.Config.Config_World1
-- Tipo: ModuleScript
...
function u1.IsWorld(p2, p3) -- Line: 10
    -- upvalues: u1 (copy)
    return u1.WORLD == p3;
end;
...
function u1.GetXPRequired(p4) -- Line: 248
    -- upvalues: u1 (copy)
...
function u1.CalculateMaxSpeed(p5) -- Line: 457
    -- upvalues: u1 (copy)
    return math.min(u1.DEFAULT_WALKSPEED + p5 * u1.SPEED_GAIN_PER_LEVEL, u1.MAX_LEVEL_SPEED_CAP);
end;
...
function u1.GetRebirthProductId(p6) -- Line: 500
    -- upvalues: u1 (copy)
    local v7 = (p6 or 0) + 1;
