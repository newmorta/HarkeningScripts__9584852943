-- Ruta Original: StarterPlayer.StarterPlayerScripts.Pièges.World3.LocalNPC_LolMonster_AI
-- Tipo: LocalScript
...
local u1 = NPC_LolMonster:GetPivot();
task.spawn(function() -- Line: 35
    -- upvalues: ContentProvider (copy), Animation (copy), Animation2 (copy), NPC_LolMonster (copy)
    ContentProvider:PreloadAsync({ Animation, Animation2, NPC_LolMonster });
end);
local u2 = {};

local function startNPCLogic(u3, p4) -- Line: 44
    -- upvalues: u2 (copy), NPC_LolMonster (copy), u1 (copy), Animation (copy), Animation2 (copy), LocalPlayer (copy)
    if u2[u3] then
        return;
    end;

    u2[u3] = true;
    local u5 = NPC_LolMonster:Clone();
    u5:PivotTo(u1);
    u5.Parent = workspace;
        ...
        while u5.Parent and u3.Parent do
            ...
            if v21 ~= u8 then
                if v21 == "Chasing" or v21 == "Returning" then
                    ...
                    Humanoid.WalkSpeed = v21 == "Chasing" and 100 or 50;
            ...
            if u8 == "Chasing" and v12 then
                if v16 > 350 then
                    HumanoidRootPart.Anchored = true;
                    u5:PivotTo(CFrame.lookAt(Position + (v13 - Position).Unit * 100 * 0.03, v13));
                else
                    HumanoidRootPart.Anchored = false;
...
for _, v in ipairs(CollectionService:GetTagged("LolMonster_AttackZone")) do
