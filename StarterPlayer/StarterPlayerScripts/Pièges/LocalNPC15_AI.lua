-- Ruta Original: StarterPlayer.StarterPlayerScripts.Pièges.LocalNPC15_AI
-- Tipo: LocalScript
...
local NPC15 = ReplicatedStorage:WaitForChild("NPC15");
local u1 = NPC15:GetPivot();
local Animation = Instance.new("Animation");
Animation.AnimationId = "rbxassetid://136780856423547";
local Animation2 = Instance.new("Animation");
Animation2.AnimationId = "rbxassetid://103924730807406";
task.spawn(function() -- Line: 42
    -- upvalues: ContentProvider (copy), Animation (copy), Animation2 (copy), NPC15 (copy)
    ContentProvider:PreloadAsync({ Animation, Animation2, NPC15 });
end);
...
local function loadWaypoints() -- Line: 54
    ...
    for _, v in ipairs(CollectionService:GetTagged("NPC15_Waypoint")) do
...
local function isInAnyZonePart(p6) -- Line: 82
    -- upvalues: CollectionService (copy)
    for _, v in ipairs(CollectionService:GetTagged("NPC15_Zone")) do
        if v:IsA("BasePart") then
...
local function isInSpeedZone(p8) -- Line: 96
    -- upvalues: CollectionService (copy)
    for _, v in ipairs(CollectionService:GetTagged("NPC15_SpeedZone")) do
        if v:IsA("BasePart") then
...
local function startNPC() -- Line: 115
    -- upvalues: u10 (ref), loadWaypoints (copy), CollectionService (copy), NPC15 (copy), u1 (copy), PathfindingService (copy), u5 (copy), Animation (copy), Animation2 (copy), LocalPlayer (copy), TweenService (copy), RunService (copy), isInAnyZonePart (copy), isInSpeedZone (copy)
    if u10 then
        return;
    end;
    ...
    if #u11 == 0 then
        ...
        if #u11 == 0 then
            warn("[NPC15] Aucun waypoint trouvé après 15s d\'attente !");
        end;
    end;

    CollectionService:GetInstanceAddedSignal("NPC15_Waypoint"):Connect(function() -- Line: 166
        -- upvalues: u11 (ref), loadWaypoints (ref)
        u11 = loadWaypoints();
    end);
    local u13 = NPC15:Clone();
    u13:PivotTo(u1);
...
local function tryStart() -- Line: 827
    ...
    if #CollectionService:GetTagged("NPC15_Zone") > 0 then
        task.spawn(startNPC);
    end;
end;

CollectionService:GetInstanceAddedSignal("NPC15_Zone"):Connect(tryStart);

if not u10 and #CollectionService:GetTagged("NPC15_Zone") > 0 then
    task.spawn(startNPC);
end;