-- Ruta Original: StarterPlayer.StarterPlayerScripts.Pièges.LocalNPC_MacaronMonster_AI
-- Tipo: LocalScript
...
local NPC_MacaronMonster = ReplicatedStorage:WaitForChild("NPC_MacaronMonster", 10);

if not NPC_MacaronMonster then
    return;
end;
...
local u1 = NPC_MacaronMonster:GetPivot();
task.spawn(function() -- Line: 35
    -- upvalues: ContentProvider (copy), Animation (copy), Animation2 (copy), NPC_MacaronMonster (copy)
    ContentProvider:PreloadAsync({ Animation, Animation2, NPC_MacaronMonster });
end);
local u2 = {};

local function startNPCLogic(u3, p4) -- Line: 44
    -- upvalues: u2 (copy), NPC_MacaronMonster (copy), u1 (copy), Animation (copy), Animation2 (copy), LocalPlayer (copy)
    if u2[u3] then
        return;
    end;

    u2[u3] = true;
    local u5 = NPC_MacaronMonster:Clone();
    u5:PivotTo(u1);
    u5.Parent = workspace;
    ...
    local u8 = "Idle";
    u7:Play();
    local u11 = Hitbox.Touched:Connect(function(p9) -- Line: 74
        ...
        while u5.Parent and u3.Parent do
            ...
            if v21 ~= u8 then
                if v21 == "Chasing" or v21 == "Returning" then
                    ...
                    Humanoid.WalkSpeed = v21 == "Chasing" and 122 or 50;
            ...
            if u8 == "Chasing" and v12 then
                if v16 > 350 then
                    HumanoidRootPart.Anchored = true;
                    u5:PivotTo(CFrame.lookAt(Position + (v13 - Position).Unit * 122 * 0.03, v13));
                else
                    HumanoidRootPart.Anchored = false;
...
local function onZoneDetected(p22) -- Line: 163
    -- upvalues: startNPCLogic (copy)
    startNPCLogic(p22, p22);
end;

CollectionService:GetInstanceAddedSignal("MacaronMonster_AttackZone"):Connect(onZoneDetected);

for _, v in ipairs(CollectionService:GetTagged("MacaronMonster_AttackZone")) do
