-- Ruta Original: StarterPlayer.StarterPlayerScripts.Pièges.LocalNPC12_AI
-- Tipo: LocalScript
...
local NPC12 = ReplicatedStorage:WaitForChild("NPC12");
local u1 = NPC12:GetPivot();
...
task.spawn(function() -- Line: 38
    -- upvalues: ContentProvider (copy), Animation (copy), Animation2 (copy), NPC12 (copy)
    ContentProvider:PreloadAsync({ Animation, Animation2, NPC12 });
end);
local u2 = {};
local u3 = {
    AgentRadius = 1,
...
local function startNPC(u8) -- Line: 70
    -- upvalues: u2 (copy), NPC12 (copy), u1 (copy), PathfindingService (copy), u3 (copy), Animation (copy), Animation2 (copy), LocalPlayer (copy), TweenService (copy), RunService (copy), isInZone (copy)
    if u2[u8] then
        return;
    end;

    u2[u8] = true;
    local u9 = NPC12:Clone();
    u9:PivotTo(u1);
    u9.Parent = workspace;
    ...
    u77 = RunService.RenderStepped:Connect(function(p78) -- Line: 378
        ...
        elseif u14 == "Chasing" then
            if not v82 and os.clock() - u17 > 2 then
                u14 = "Returning";
                u18 = {};
                u19 = 0;
                u21 = 0;
...
CollectionService:GetInstanceAddedSignal("NPC12_LabyrinthZone"):Connect(onZone);

for _, v in ipairs(CollectionService:GetTagged("NPC12_LabyrinthZone")) do
    task.spawn(onZone, v);
end;